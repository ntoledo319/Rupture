...
REMAP_TABLE: Dict[str, RemapEntry] = {
    ...
    "docker":               RemapEntry("docker", "docker",                "extras_to_dnf",
        note="AL2 used `amazon-linux-extras install docker`. AL2023: `dnf install docker`."),
    "nginx1":               RemapEntry("nginx1", "nginx",                 "renamed",
        note="AL2 extras `nginx1` → AL2023 `nginx` (mainline)."),
    "php8.1":               RemapEntry("php8.1", "php8.2",                "replaced_by",
        note="AL2 extras had php up to 8.1. AL2023 ships php8.2 in mainline; php8.1 removed."),
    "php8.0":               RemapEntry("php8.0", "php8.2",                "replaced_by",
        note="Upgrade to php8.2. php8.0 is EOL upstream and not in AL2023."),
    "php7.4":               RemapEntry("php7.4", "php8.2",                "replaced_by",
        note="PHP 7.4 is upstream EOL. AL2023 has php8.2 only."),
    "python3.8":            RemapEntry("python3.8", "python3.11",         "replaced_by",
        note="AL2023 default python is 3.11. 3.8 not available."),
    "python3.7":            RemapEntry("python3.7", "python3.11",         "replaced_by",
        note="Upstream EOL. Migrate code to 3.11."),
    "postgresql14":         RemapEntry("postgresql14", "postgresql15",    "replaced_by",
        note="AL2023 ships postgresql15 in mainline."),
    "postgresql13":         RemapEntry("postgresql13", "postgresql15",    "replaced_by",
        note="PG 13 upstream EOL Nov 2025. AL2023 ships 15."),
    "mariadb10.5":          RemapEntry("mariadb10.5", "mariadb105",       "renamed",
        note="Package naming convention changed; same software."),
    "redis6":               RemapEntry("redis6", "redis6",                "extras_to_dnf",
        note="Now in mainline. Just `dnf install redis6`."),
    "memcached1.5":         RemapEntry("memcached1.5", "memcached",       "renamed",
        note="AL2023: single `memcached` package."),
    "golang":               RemapEntry("golang", "golang",                "extras_to_dnf",
        note="Use `dnf install golang` directly."),
    "rust1":                RemapEntry("rust1", "rust",                   "renamed", ""),
    "ruby3.0":              RemapEntry("ruby3.0", "ruby3.2",              "replaced_by",
        note="AL2023 default is 3.2. Ruby 3.0 upstream EOL."),
    "ruby2.7":              RemapEntry("ruby2.7", "ruby3.2",              "replaced_by",
        note="EOL. Migrate code."),
    "nodejs14":             RemapEntry("nodejs14", "nodejs20",            "replaced_by",
        note="Node 14 EOL. AL2023 ships 20 LTS."),
    "nodejs12":             RemapEntry("nodejs12", "nodejs20",            "replaced_by",
        note="Node 12 EOL."),
    "tomcat9":              RemapEntry("tomcat9", None,                   "removed",
        note="No longer packaged. Install from upstream or use a different servlet container."),
    "corretto8":            RemapEntry("corretto8", "java-17-amazon-corretto",  "replaced_by",
        note="AL2023 ships Corretto 17 by default. Corretto 8 via `dnf install java-1.8.0-amazon-corretto`."),
    "java-openjdk":         RemapEntry("java-openjdk", "java-17-amazon-corretto", "replaced_by",
        note="AL2023 replaces OpenJDK builds with Corretto."),

    # Removed/absent in AL2023
    "ntp":                  RemapEntry("ntp", "chrony",                   "replaced_by",
        note="ntpd is removed. AL2023 uses chrony for time sync."),
    "python-pip":           RemapEntry("python-pip", "python3-pip",       "renamed",
        note="Python 2 is gone. Use the Python 3 pip."),
    "python-virtualenv":    RemapEntry("python-virtualenv", "python3-virtualenv", "renamed"),
    "yum-utils":            RemapEntry("yum-utils", "dnf-utils",          "renamed",
        note="DNF replaces YUM. `yum-config-manager` is now `dnf config-manager`."),
    "yum-cron":             RemapEntry("yum-cron", "dnf-automatic",       "replaced_by",
        note="Automatic updates now via dnf-automatic.timer."),
    "iptables-services":    RemapEntry("iptables-services", "iptables-legacy", "renamed",
        note="nftables is default. Install iptables-legacy for compat."),
    "screen":               RemapEntry("screen", "screen",                "same",
        note="Still available via `dnf install screen`."),
    "httpd24":              RemapEntry("httpd24", "httpd",                "renamed",
    ...
    "gcc72":                RemapEntry("gcc72", "gcc",                    "replaced_by",
        note="AL2023 ships gcc 11 by default."),
    ...
    "mysql":                RemapEntry("mysql", "mariadb105",             "replaced_by",
    ...
    "curl":                 RemapEntry("curl", "curl-minimal",            "renamed",
...
def remap_package_list(packages: List[str]) -> List[RemapEntry]:
    ...
    for p in packages:
        ...
        if entry:
            ...
            out.append(RemapEntry(al2_name=p, al2023_name=p, category="unknown",
...
def _render_table(entries: List[RemapEntry]) -> str:
    ...
    c2 = max(len("AL2023 EQUIVALENT"), max((len(e.al2023_name or "—") for e in entries), default=0))
    ...
    for e in entries:
        ...
        lines.append(f"{e.al2_name:<{c1}}  {(e.al2023_name or '—'):<{c2}}  {e.category:<{c3}} {marker} {e.note}")
    return "\n".join(lines)


def _render_markdown(entries: List[RemapEntry]) -> str:
    lines = ["| AL2 package | AL2023 equivalent | Category | Action | Note |",
             "|-------------|-------------------|----------|--------|------|"]
    for e in entries:
        act = "⚠️" if e.action_required else "✓"
        note = e.note.replace("|", "\\|")
        lines.append(f"| `{e.al2_name}` | `{e.al2023_name or '—'}` | {e.category} | {act} | {note} |")
    return "\n".join(lines)
